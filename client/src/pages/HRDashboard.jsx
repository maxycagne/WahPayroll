...
export default function HRDashboard() {
  ...
  const updateDocsMutation = useMutation({
    ...
    onError: (error) => showToast(error.message || "Error updating documents", "error"),
  });
  ...
  const updateResignationMutation = useMutation({
    ...
    onError: (error) => showToast(error.message || "Error updating resignation", "error"),
  });
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="m-0 font-semibold text-gray-900 whitespace-nowrap">View Employees</p>
          ...
          >
            <p className="m-0 text-2xl mb-2">📋</p>
            <p className="m-0 font-semibold text-gray-900 whitespace-nowrap">Attendance</p>
            <p className="m-0 text-xs text-gray-600 mt-1">
          ...
          >
            <p className="m-0 text-2xl mb-2">📅</p>
            <p className="m-0 font-semibold text-gray-900 whitespace-nowrap">Leave Requests</p>
            <p className="m-0 text-xs text-gray-600 mt-1">
                    ...
                    <td className="px-6 py-3 text-sm">
                      {r.status === "Pending Approval" && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => setConfirmAction({ type: "resignation", id: r.id, status: "approve", name: `${r.first_name} ${r.last_name}` })}
                          ...
                          <button
                            onClick={() => setConfirmAction({ type: "resignation", id: r.id, status: "reject", name: `${r.first_name} ${r.last_name}` })}
                            disabled={updateResignationMutation.isPending}
                    ...
                    >
                      ...
                      {r.status === "Pending Approval" && (
                        <div className="flex gap-2 pt-3 border-t border-gray-200 mt-1">
                          <button
                            onClick={() => setConfirmAction({ type: "resignation", id: r.id, status: "Approved", name: `${r.first_name} ${r.last_name}` })}
                            disabled={updateResignationMutation.isPending}
                          ...
                          <button
                            onClick={() => setConfirmAction({ type: "resignation", id: r.id, status: "Denied", name: `${r.first_name} ${r.last_name}` })}
                            disabled={updateResignationMutation.isPending}
            ...
            <div className="p-6 space-y-4">
              <p className="m-0 text-gray-700">
                ...
                {confirmAction.type === "updateDocuments" && (
                  <>
                    <span>Are you sure you want to update the missing requirement</span>
                    <span className="block font-semibold text-purple-600 mt-2">
                      {confirmAction.missing_docs && confirmAction.missing_docs.length > 0
                        ? confirmAction.missing_docs.join(", ")
                        : "to clear all requirements"}
                    </span>
                    <span className="block mt-2">for <span className="font-semibold text-purple-600">{confirmAction.employee_name}</span>?</span>
                  </>
                )}
              </p>
