...
export default function SalaryHistory() {
  ...
  const getTotalChange = () => {
    return salaryRecords.reduce((sum, record) => {
      return sum + (record.type === 'Decrease' ? -record.amount : record.amount)
    }, 0)
  }
  ...
  return (
    <div className="max-w-full">
      <h1 className="m-0 text-[1.4rem] font-bold text-white mb-6">Salary History</h1>

      {/* Employee Selector */}
      <div className="mb-8 rounded-lg border border-gray-200 bg-white shadow-sm p-6">
        <label className="block text-sm font-semibold text-gray-900 mb-3">Select Employee</label>
        <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="rounded-lg border-l-4 border-l-blue-500 border border-gray-200 bg-white shadow-sm p-6">
          <p className="m-0 text-sm font-medium text-gray-600 mb-2">Current Salary</p>
          <p className="m-0 text-3xl font-bold text-blue-600">₱{(25000).toLocaleString()}</p>
        ...
        <div className="rounded-lg border-l-4 border-l-green-500 border border-gray-200 bg-white shadow-sm p-6">
          <p className="m-0 text-sm font-medium text-gray-600 mb-2">Total Increases</p>
          <p className="m-0 text-3xl font-bold text-green-600">+₱{getTotalIncreases().toLocaleString()}</p>
          <p className="m-0 text-xs text-gray-500 mt-2">{getIncreaseCount()} adjustments</p>
        </div>

        <div className={`rounded-lg border-l-4 ${getTotalChange() > 0 ? 'border-l-green-500' : 'border-l-red-500'} border border-gray-200 bg-white shadow-sm p-6`}>
          <p className="m-0 text-sm font-medium text-gray-600 mb-2">Net Change</p>
          <p className={`m-0 text-3xl font-bold ${getTotalChange() > 0 ? 'text-green-600' : 'text-red-600'}`}>
            {getTotalChange() > 0 ? '+' : ''}₱{getTotalChange().toLocaleString()}
          </p>
      ...
      <section className="rounded-lg border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-6 py-4">
          <h3 className="m-0 text-lg font-semibold text-gray-900">Salary Adjustment History</h3>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">New Salary</th>
              ...
              {salaryRecords.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td className="px-6 py-3 text-sm">
                    <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${typeColors[record.type].bg} ${typeColors[record.type].text}`}>
                  ...
                  <td className="px-6 py-3 text-sm text-right font-semibold">
                    <span className={record.type === 'Decrease' ? 'text-red-600' : 'text-green-600'}>
                      {record.type === 'Decrease' ? '-' : '+'}₱{record.amount.toLocaleString()}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-sm text-right font-bold text-gray-900">₱{record.newSalary.toLocaleString()}</td>
                  <td className="px-6 py-3 text-sm text-gray-600">{record.remarks}</td>
                </tr>
              ))}
            </tbody>
          </table>
