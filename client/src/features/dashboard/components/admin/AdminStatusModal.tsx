...
type AdminStatusModalProps = {
  ...
  dashboardData: AdminDashboardSummaryData;
  approvedLeaves: Set<string | number>;
  priorityOrder: Record<LeavePriority, number>;
  priorityClass: StatusBadgeClass;
  onClose: () => void;
  onOpenLeaveDecisionConfirm: (
...
export default function AdminStatusModal({
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-violet-200 bg-linear-to-r from-[#4e1b8a] to-[#6630b0] px-4 py-3">
          ...
          {activeModal === "pending" && (
            <div className="space-y-2.5">
              {[...(dashboardData.pendingLeaves || [])]
                .sort((a, b) => {
                  const aPriority =
                    priorityOrder[(a.priority as LeavePriority) || "Low"] ??
                    Number.MAX_SAFE_INTEGER;
                  const bPriority =
                    priorityOrder[(b.priority as LeavePriority) || "Low"] ??
                    Number.MAX_SAFE_INTEGER;
                      ...
                      {!approvedLeaves.has(employee.id) ? (
                        <>
                          <button
                            type="button"
                            onClick={() =>
                              onOpenLeaveDecisionConfirm(employee, "Approved")
                            }
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              onOpenLeaveDecisionConfirm(employee, "Denied")
                            }
                        ...
                        <span className="inline-flex items-center whitespace-nowrap rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800">
                          Processed
                        </span>
                      )}
                    </div>
